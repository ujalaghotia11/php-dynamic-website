...
// Check if form is submitted to update the order status
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($order_status == 'Cancelled') {
        // If cancelled, add a cancellation reason
        $cancel_reason = mysqli_real_escape_string($conn, $_POST['cancel_reason']);
        $update_sql = "UPDATE orders SET order_status = '$order_status', cancel_reason = '$cancel_reason' WHERE order_id = $order_id";
    } else {
    ...
    if (mysqli_query($conn, $update_sql)) {
        // Process commissions if status is Approved
        if ($order_status == 'Approved') {
            // Fetch order details
            $fetch_order_sql = "SELECT user_id, order_amount FROM orders WHERE order_id = ?";
            $stmt = $conn->prepare($fetch_order_sql);
            if ($stmt) {
                $stmt->bind_param("i", $order_id);
                $stmt->execute();
                $stmt->bind_result($user_id, $order_amount);
                $stmt->fetch();
                $stmt->close();

                // Fetch referral code for the user
                $referral_sql = "SELECT referral_code FROM users WHERE id = ?";
                $referral_stmt = $conn->prepare($referral_sql);
                if ($referral_stmt) {
                    $referral_stmt->bind_param("i", $user_id);
                    $referral_stmt->execute();
                    $referral_stmt->bind_result($referral_code);
                    $referral_stmt->fetch();
                    $referral_stmt->close();

                    // Process referral commissions if referral code exists
                    if (!empty($referral_code)) {
                        trackReferral($referral_code, $user_id, $order_amount);
                    }
                } else {
                    echo "Error fetching referral code: " . $conn->error;
                    exit();
                }
            } else {
        ...
        header("Location: orderdetails.php?success=Order status updated successfully");
    } else {
        // Redirect with an error message
        header("Location: orderdetails.php?error=Failed to update order status");
    }
    exit();
}
    ...
    if ($result->num_rows > 0) {
        ...
        while ($referred_by_user_id != NULL) {
            ...
            if ($commission_percentage > 0) {
                ...
                if (!$stmt) {
                    die("Error inserting commissions: " . $conn->error);
                }
                $stmt->bind_param("iiid", $referred_by_user_id, $user_id, $commission_amount, $referral_level);
                if (!$stmt->execute()) {
                    die("Error executing commission insert: " . $stmt->error);
                }

                // Update total commissions in users table
            ...
            if (!$stmt) {
                die("Error fetching referrer: " . $conn->error);
            }
            ...
            $referral_level++;
            if ($referral_level > 10) break; // Limit chain to 10 levels
        }
    }
}
...
function getCommissionForLevel($level) {
    ...
    if (!$stmt) {
        die("Error getting commission percentage: " . $conn->error);
    }
...
function updateTotalCommission($user_id, $commission_amount) {
    ...
    if (!$stmt) {
        die("Error updating total commission: " . $conn->error);
    }
...
<body class="g-sidenav-show   bg-gray-100">
  <div class=" bg-dark position-absolute w-100 h-100"></div>
      ...
      <ul class="navbar-nav">
        ...
        <li class="nav-item dropdown">
          ...
          <ul class="dropdown-menu" aria-labelledby="tablesDropdown">
            ...
            <li><a class="dropdown-item" href="../pages/logo.php">logo</a></li>
            
            <li><a class="dropdown-item" href="../pages/topbanner.php">Top-Banner</a></li>
            <li><a class="dropdown-item" href="../pages/mid_banner.php">Mid-Banner</a></li>
            <li><a class="dropdown-item" href="../pages/manageproduct.php">Manage Product</a></li>
            <li><a class="dropdown-item" href="../pages/orderdetails.php">Order Details</a></li>
            <li><a class="dropdown-item" href="../pages/product_images.php">Product Images</a></li>
            <li><a class="dropdown-item" href="../pages/payment_method.php">Payment Methods</a></li>
          </ul>
        </li>
                ...
                <table class="table align-items-center justify-content-center mb-0">
                    <thead>
                        <tr>
                            ...
                            <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 ps-2">UTR Number</th>
                    ...
                    <tbody>
                        ...
                        while ($order = mysqli_fetch_assoc($result)) { ?>
                            <tr>
                                <td>
                                    <span class="text-xs font-weight-bold"><?php echo $order['order_id']; ?></span>
                                </td>
                                <td>
                                    <span class="text-xs font-weight-bold"><?php echo $order['product_name']; ?></span>
                                </td>
                                <td>
                                    <span class="text-xs font-weight-bold"><?php echo $order['customer_name']; ?></span>
                                </td>
                                <td>
                                    <span class="text-xs"><?php echo $order['phone']; ?></span>
                                </td>
                                <td>
                                    <span class="text-xs"><?php echo $order['utr_number']; ?></span>
                                </td>
                                <td>
                                ...
                                <td>
                                    ...
                                    <form action="update_order_status.php" method="POST" class="form-inline">
                                        ...
                                        <select name="order_status" class="form-select" id="status-<?php echo $order['order_id']; ?>" required>
                                    ...
                                    <!-- Modal Trigger for Cancellation Reason -->
                                    <button class="btn btn-danger btn-sm mt-2" data-bs-toggle="modal" data-bs-target="#cancelModal<?php echo $order['order_id']; ?>">Cancel</button>
                                            ...
                                            <div class="modal-content">
                                                ...
                                                <div class="modal-body">
                                                    <form action="update_order_status.php" method="POST">
                                                        ...
                                                        <textarea name="cancel_reason" class="form-control" placeholder="Enter cancellation reason" required></textarea>
